...
function view(row: Row) {
  ...
  const registrationComplete = Boolean(row.learner_name?.trim() && row.class_name?.trim() && row.phone?.trim() && row.registration_submitted_at);
  ...
  try {
      ...
      await database.batch([
        database.prepare(
          `INSERT INTO access_automation_settings
            (id, auto_confirm_new_devices, default_access_days, updated_by, updated_at)
        ...
        ).bind(enabled ? 1 : 0, defaultAccessDays, actor),
        database.prepare(
          "INSERT INTO course_audit_log (actor, action, target, detail_json) VALUES (?, 'access_automation_updated', 'global', ?)",
        ).bind(actor, JSON.stringify({ enabled, defaultAccessDays, source: "control-center" })),
      ]);
    ...
    const current = await database.prepare(
      `SELECT device_id, display_code, learner_name, learner_family_name, learner_given_name,
              class_name, phone, registration_submitted_at,
    ...
    const registrationComplete = Boolean(current.learner_name?.trim() && current.class_name?.trim() && current.phone?.trim() && current.registration_submitted_at);
    if (["grant-free", "require-payment", "verify-payment"].includes(action) && !registrationComplete) {
      return respond({ error: "Thiết bị chưa gửi đủ Họ tên, Lớp và Số điện thoại." }, 409);
    }
