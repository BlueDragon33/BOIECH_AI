  ...
  deviceSchemaReady ??= database.batch([
    ...
    database.prepare(
      `INSERT INTO access_automation_settings
        (id, auto_confirm_new_devices, default_access_days)
       VALUES ('global', 1, 60)
...
function publicState(row: DeviceAccessRow): PublicDeviceState {
  ...
  return {
    ...
    className,
    phone,
    registrationComplete: Boolean(learnerName && className && phone && row.registration_submitted_at),
...
const deviceColumns = `device_id, display_code, public_key_jwk, status, label, learner_name,
  learner_family_name, learner_given_name,
...
export async function getAccessAutomationSettings(): Promise<AccessAutomationSettings> {
  ...
  const row = await database.prepare(
    `SELECT auto_confirm_new_devices, default_access_days, updated_by, updated_at
       FROM access_automation_settings WHERE id = 'global'`,
  ).first<{ auto_confirm_new_devices: number; default_access_days: number; updated_by: string | null; updated_at: string | null }>();
  return {
...
export async function verifyDeviceIdentityRequest(payload: Record<string, unknown>, skipSignatureCheck = false) {
  ...
  if (!proof || proof.expires_at < Date.now()) {
    throw new DeviceAccessError("Phiên xác thực thiết bị đã hết hạn.", 401, "DEVICE_PROOF_EXPIRED");
  }
...
export async function saveDeviceRegistration(payload: Record<string, unknown>, skipSignatureCheck = false) {
  ...
  const current = await database.prepare(
    `SELECT learner_name, learner_family_name, learner_given_name, class_name, phone,
  ...
  try {
    await database.prepare(
      `UPDATE device_access SET learner_name = ?, learner_family_name = ?, learner_given_name = ?, class_name = ?, phone = ?,
       ...
       WHERE device_id = ?`,
    ).bind(learnerName, learnerFamilyName, learnerGivenName, className, phone, device.deviceId).run();
    if (shouldAutoConfirm) {
      await database.prepare(
        `UPDATE device_access SET status = 'approved', access_group = 'free', payment_status = 'free_approved',
                approved_at = CURRENT_TIMESTAMP, blocked_at = NULL, personal_edit_enabled = 1,
                auto_confirmed_at = CURRENT_TIMESTAMP, access_expires_at = ?, updated_at = CURRENT_TIMESTAMP
          WHERE device_id = ?`,
      ).bind(accessExpiryAfterDays(automation.defaultAccessDays), device.deviceId).run();
    }
  } catch (error) {
    if (error instanceof Error && /device_access\.phone|device_access_phone_unique|UNIQUE constraint failed/i.test(error.message)) {
  ...
  if (changed) {
    ...
    ).bind(device.deviceCode, device.deviceId, JSON.stringify({ learnerName, learnerFamilyName, learnerGivenName, className, phone })).run();
  }
  if (shouldAutoConfirm) {
    await database.prepare(
      "INSERT INTO course_audit_log (actor, action, target, detail_json) VALUES (?, 'learning_device_auto_confirmed', ?, ?)",
    ).bind("automation", device.deviceId, JSON.stringify({ accessDays: automation.defaultAccessDays, personalEditEnabled: true })).run();
  }
