...
async function issueCertificate(credential: StoredDeviceCredential, device: DeviceAccessState) {
  ...
  const data = await response.json() as { certificate?: CourseCertificate; error?: string };
  if (!response.ok || !data.certificate) throw new CourseApiError(data.error ?? "Không thể tạo chứng chỉ hoàn thành.");
...
function DeviceGate({
  ...
  async function submitRegistration(event: React.FormEvent<HTMLFormElement>) {
    ...
    setSubmitting(true);
    setLocalError("");
    try { await onRegister({ learnerFamilyName, learnerGivenName, className, phone }); }
      ...
      <section>
        ...
        {needsRegistration ? (
          <form className="device-registration-form" onSubmit={(event) => void submitRegistration(event)}>
            ...
            <label><span>Lớp</span><input value={className} onChange={(event) => setClassName(event.target.value)} placeholder="Ví dụ: 7A1" maxLength={50} required /></label>
...
export default function Home() {
  ...
  const activeContent = deviceAccess?.personalEditEnabled && localContent?.lessonNumber === selectedLesson
  ...
  useEffect(() => {
    if (!hasEnteredLearning || deviceAccess?.status !== "approved" || deviceAccess.accessExpired) return;
    const recordMinute = () => {
      ...
      if (offline) {
        const clientEventId = `offline:${crypto.randomUUID()}`;
        void enqueueOfflineAction({
          ...
          occurredAt,
          clientEventId,
        }).then(() => refreshPendingSyncCount()).catch(() => undefined);
      }
    };
    const timer = window.setInterval(recordMinute, 60_000);
    return () => window.clearInterval(timer);
  }, [deviceAccess?.accessExpired, deviceAccess?.status, hasEnteredLearning, section, selectedLesson]);
  ...
  useEffect(() => {
    if (!deviceCredential || deviceAccess?.status !== "approved") return;
    const sendHeartbeat = () => {
    ...
    return () => window.clearInterval(timer);
  // Heartbeats use the current action queue without restarting the timer for each render.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [deviceAccess, deviceCredential, section, selectedLesson]);
      ...
      <main className="main-area">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            {installPrompt ? <button className="install-app" onClick={() => void installWebApp()}>Cài ứng dụng</button> : null}
            <div className="learner-chip" title={learnerFullName}><span>{learnerInitial}</span><div><small>Học viên</small><strong>Chào {learnerGivenName}</strong></div></div>
            ...
            {deviceAccess.personalEditEnabled && officialContent ? <button className="personal-edit-trigger" onClick={() => setLocalStudioOpen(true)}><span>{localContent ? "Đang dùng bản riêng" : "Sửa bản riêng"}</span><b>Bài {selectedLesson}</b></button> : null}
            <a className="editor-request-link" href={`/bien-tap-noi-dung?lesson=${selectedLesson}`}><span>Xin quyền chỉnh sửa</span><b>Bài {selectedLesson}</b></a>
