...
async function loadPoseLandmarker(): Promise<{ landmarker: PoseLandmarker; cleanup: () => void }> {
  ...
  const [bundleResponse, loaderResponse, wasmResponse, modelResponse] = await Promise.all([
    cachedAsset(VISION_BUNDLE_URL), cachedAsset(WASM_LOADER_URL), cachedAsset(WASM_BINARY_URL), cachedAsset(POSE_MODEL_URL),
  ]);
...
function detectErrors(frames: FrameMetric[], cameraView: CameraView, confidence: number) {
  ...
  if (cameraView === "rear") {
    ...
    if (bad.length / frames.length >= 0.12) {
      ...
      errors.push(makeError({
        code: "KNEE_SPREAD_HIGH", title: "Hai gối mở rộng kéo dài khi thu chân", timeSec: worst.frame.timeSec,
        severity: severe(bad.length / frames.length), metric: "knee_spread_ratio", observed: `${worst.frame.kneeSpread.toFixed(2)}× bề rộng hông`,
        ...
        frameIndex: worst.index, highlight: [23, 24, 25, 26, 27, 28],
      }, confidence));
    }
  }
  ...
  if (legBad.length / frames.length >= 0.14) {
    ...
    errors.push(makeError({
      code: "LEG_ASYMMETRY", title: "Hai chân gập không đồng đều", timeSec: worst.frame.timeSec,
      severity: severe(legBad.length / frames.length), metric: "knee_angle_delta", observed: `Lệch ${worst.frame.kneeDiff.toFixed(0)}°`,
  ...
  if (armBad.length / frames.length >= 0.14) {
    ...
    errors.push(makeError({
      code: "ARM_ASYMMETRY", title: "Hai tay thu/duỗi thiếu đối xứng", timeSec: worst.frame.timeSec,
      severity: severe(armBad.length / frames.length), metric: "elbow_angle_delta", observed: `Lệch ${worst.frame.armDiff.toFixed(0)}°`,
      ...
      frameIndex: worst.index, highlight: [11, 12, 13, 14, 15, 16],
    }, confidence));
  }
  ...
  if (overlapFrames.length / frames.length >= 0.18) {
    ...
    errors.push(makeError({
      code: "ARM_LEG_OVERLAP", title: "Tay và chân có dấu hiệu chồng pha", timeSec: worst.frame.timeSec,
      severity: severe(overlapFrames.length / frames.length), metric: "arm_leg_overlap",
      ...
      frameIndex: worst.index, highlight: [15, 16, 25, 26, 27, 28],
    }, confidence));
  }
  ...
  if (cameraView === "side") {
    ...
    if (bodyBad.length / frames.length >= 0.2) {
      ...
      errors.push(makeError({
        code: "BODY_LINE_TILT", title: "Trục vai–hông thay đổi lớn so với phương ngang", timeSec: worst.frame.timeSec,
        severity: severe(bodyBad.length / frames.length), metric: "body_tilt", observed: `${worst.frame.bodyTilt.toFixed(0)}°`,
        ...
        frameIndex: worst.index, highlight: [11, 12, 23, 24],
      }, confidence));
    }
  }
...
export default function VideoAnalyzer({ lessonNumber = "03" }: { lessonNumber?: string }) {
  ...
  const flushPending = useCallback(async () => {
    ...
    try {
      ...
      // Giữ hàng đợi local; sự kiện online tiếp theo sẽ thử lại.
    }
  }, [lessonNumber]);
  ...
  function selectFile(next: File | null) {
    ...
    if (next.size > MAX_VIDEO_BYTES) { setStatus(`Video ${formatBytes(next.size)} vượt giới hạn 50 MB.`); return; }
  ...
  async function analyze() {
    ...
    try {
      ...
      const analysis: VideoAnalysisResult = {
        ...
        width: video.videoWidth, height: video.videoHeight, sampledFrames: totalFrames, detectedFrames: frames.length,
        cameraView, score, confidence, categories, errors, localFrames: annotated, syncState: navigator.onLine ? "pending" : "local",
      };
  ...
  return (
    <div className={styles.shell}>
      <section className={styles.hero}>
        <div><span>Breaststroke Vision · chạy trên thiết bị</span><h1>Phân tích video kỹ thuật bơi ếch</h1><p>Video được đọc trực tiếp trong trình duyệt. Máy chủ không nhận video gốc; chỉ nhận điểm, lỗi, thời điểm và các chỉ số sau khi AI xử lý.</p></div>
      ...
      <section className={styles.workbench}>
        <div className={styles.videoPanel}>
          ...
          {objectUrl ? <video ref={videoRef} className={styles.studentVideo} src={objectUrl} controls preload="metadata" playsInline muted onLoadedMetadata={() => {
            const video = videoRef.current;
            if (video && video.duration > MAX_VIDEO_SECONDS + 0.05) setStatus(`Video dài ${video.duration.toFixed(1)} giây; hãy chọn đoạn tối đa 30 giây.`);
