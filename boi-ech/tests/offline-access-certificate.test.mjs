...
test("auto-confirms complete new profiles as free and limits local editing to teachers", async () => {
...
test("keeps personal lesson edits local and outside every publish workflow", async () => {
  ...
  assert.match(page, /sameLessonContent/);
  assert.match(page, /Bản máy chủ/);
  assert.match(studio, /Khôi phục bản máy chủ/);
...
test("records active offline study idempotently and syncs it when connectivity returns", async () => {
  ...
  assert.match(page, /const clientEventId = `study:\$\{crypto\.randomUUID\(\)\}`/);
  assert.match(page, /offlinePayload[\s\S]*?clientEventId,[\s\S]*?action: "heartbeat"[\s\S]*?clientEventId,/);
