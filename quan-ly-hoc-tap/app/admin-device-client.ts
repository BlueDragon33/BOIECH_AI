...
function openDb() {
  return new Promise<IDBDatabase>((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      if (!request.result.objectStoreNames.contains("credential")) {
        request.result.createObjectStore("credential");
      }
    };
...
async function jsonApi(path: string, body: Record<string, unknown>) {
  ...
  const data = await response.json().catch(() => ({
    error: "Phản hồi quản trị không hợp lệ.",
  })) as ApiPayload;
  if (!response.ok) {
    throw new AdminApiError(data.error ?? "Không thể kết nối dịch vụ quản trị.", data);
  }
  return data;
}

async function register(credential: Credential) {
  const data = await jsonApi("/api/device", {
    action: "register",
    publicKey: credential.publicKey,
  });
  if (!data.device) {
    throw new AdminApiError("Máy chủ chưa trả về trạng thái thiết bị quản trị.", data);
  }
  return data.device;
}

async function proof(credential: Credential, access: AdminAccess) {
  const challenge = await jsonApi("/api/device", {
    action: "challenge",
  ...
  const message = new TextEncoder().encode(
    `learning-control:${access.deviceId}:${challenge.challenge}`,
  );
...
async function secureApi(
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      return await jsonApi(path, {
        ...body,
        ...await proof(credential, access),
      });
    } catch (error) {
      lastError = error;
      if (!(error instanceof AdminApiError) || error.data.code !== "DEVICE_PROOF_EXPIRED") {
        throw error;
      }
    }
  }
...
function currentApplication() {
  if (typeof window !== "undefined" && window.location.pathname.startsWith("/apps/suc-khoe-tre")) {
    return "child-health";
  }
  if (typeof window !== "undefined" && window.location.pathname.startsWith("/apps/bauman-master-ai")) {
...
export async function connectAdminCenter() {
  ...
  const bootstrap = await secureApi("/api/center", credential, access, {
    action: "bootstrap",
  }) as CenterBootstrap;
...
export async function upstreamJson<T>(
  ...
  if (!/^\/api\/control\/[a-z0-9-]+$/i.test(path)) {
    throw new Error("Đường dẫn quản trị ứng dụng không hợp lệ.");
  }
  ...
  const data = await response.json().catch(() => ({
    error: "Phản hồi ứng dụng không hợp lệ.",
  })) as T & { error?: string };
  if (!response.ok) {
    throw new Error(data.error ?? "Không thể kết nối ứng dụng.");
  }
  return data as T;
}
