...
export type ClientDevicePolicy = {
  applicationId: string;
...
export type ClientDeviceActionOutcome = {
  applicationId: string;
  deviceId: string;
  deviceCode?: string;
...
export type ClientDeviceApiResponse = {
  devices: ClientDeviceView[];
  sources: ClientDeviceSource[];
  policies: ClientDevicePolicy[];
  syncedAt: string;
  notice?: string;
  error?: string;
  code?: string;
  adminHref?: string | null;
  automationOutcomes?: ClientDeviceActionOutcome[];
  bulkOutcomes?: ClientDeviceActionOutcome[];
};
...
export type CenterApiResponse = Partial<CenterBootstrap> & { error?: string; code?: string };

type Credential = { version: 1; privateKey: CryptoKey | null; publicKey: JsonWebKey };
type ApiPayload = Partial<AdminBootstrap> & Partial<CenterBootstrap> & Partial<ClientDeviceApiResponse> & {
  device?: AdminAccess;
  challenge?: string;
  error?: string;
  code?: string;
...
export function bridgeForApplication(bootstrap: AdminBootstrap) { return bootstrap.applicationBridge ?? bootstrap.boiBridge; }
...
export async function clientDeviceAdminAction(body: Record<string, unknown>) {
  const credential = await credentialForDevice();
  const access = await register(credential);
  if (access.status !== "approved") throw new AdminApiError("Thiết bị quản trị chưa được cấp quyền.", { device: access });
  return await secureApi("/api/client-devices", credential, access, body) as ClientDeviceApiResponse;
}
...
export async function connectAdminDevice(application = currentApplication()) {
  ...
  const dashboardPath = application === "child-health" ? "/api/dashboard-health"
    : application === "bauman-master-ai" ? "/api/dashboard-bauman"
      : application === "ru-life" ? "/api/dashboard-ru-life"
        : "/api/dashboard";
...
export const roleLabels: Record<ControlRole, string> = { viewer: "Chỉ xem", reviewer: "Kiểm duyệt viên", publisher: "Người xuất bản", owner: "Chủ hệ thống" };
