...
const futureClients: readonly ClientEntry[] = [
  {
    ...
    scope: "Client độc lập; production cũ chưa có adapter Hòa nhập Nga nên Trung tâm không dựng thao tác giả.",
  },
  {
    ...
    scope: "Client độc lập; production cũ chưa có admin backend GrowUP nên chỉ hiển thị trạng thái contract.",
  },
];
...
const viewTitles: Record<CenterView, { eyebrow: string; title: string; description: string }> = {
  overview: { eyebrow: "CONTROL PLANE · OPERATIONS", title: "Bảng điều phối quản trị ứng dụng", description: "Một đầu não quản trị, nhiều client độc lập. Thông tin được trình bày compact để nhìn nhanh việc cần xử lý và đi thẳng vào đúng ứng dụng." },
  inbox: { eyebrow: "PRIORITY INBOX", title: "Hộp việc ưu tiên", description: "Tập trung những việc đang chờ xử lý từ control-plane và các client production hiện có." },
  applications: { eyebrow: "CLIENT REGISTRY", title: "Ứng dụng đang quản lý", description: "Một hàng cho mỗi client; không dùng card lớn lặp thông tin và không mở thẳng runtime học tập từ Trung tâm." },
  "client-devices": { eyebrow: "CLIENT DEVICE ALERTS", title: "Thiết bị mới theo ứng dụng", description: "Trung tâm chỉ đọc số cảnh báo mà client chủ động trả về; registry thiết bị vẫn thuộc từng ứng dụng." },
  alerts: { eyebrow: "OPERATIONS ALERTS", title: "Cảnh báo vận hành", description: "Các client chưa nối production, client tạm dừng hoặc mất kết nối được đưa vào một hàng đợi rõ ràng." },
  devices: { eyebrow: "CONTROL ACCESS", title: "Thiết bị quản trị Trung tâm", description: "Chỉ chứa thiết bị QT của Application Management, tuyệt đối không trộn thiết bị người dùng của client." },
  audit: { eyebrow: "SYSTEM AUDIT", title: "Nhật ký hệ thống", description: "Audit quyền và bảo mật của control-plane. Audit nghiệp vụ vẫn nằm trong từng client." },
...
function stateLabel(state: ConnectionState) {
  return state === "connected" ? "Kết nối tốt" : state === "unavailable" ? "Không đọc được" : state === "pending" ? "Chờ backend" : "Cần hoàn thiện";
}
...
export default function ApplicationHub({ user }: { user: { displayName: string; email: string } }) {
  ...
  const workItems = useMemo<WorkItem[]>(() => {
    ...
    if (centralCounts.pending > 0) items.push({ id: "qt-pending", client: { ...clients[0], id: "control-plane", shortName: "Application Management", name: "Application Management", icon: "QT", href: null, group: "Hệ thống", boundary: "Thiết bị quản trị Trung tâm", scope: "Thiết bị QT mới đang chờ owner xử lý." }, title: "Thiết bị quản trị mới chờ duyệt", detail: `${centralCounts.pending} thiết bị QT cần quyết định`, state: "high", count: centralCounts.pending });
  ...
  const applicationTable = <div className={styles.applicationTable}>
    <div className={styles.applicationHead}><span>Ứng dụng</span><span>Nhóm nghiệp vụ</span><span>Thiết bị chờ</span><span>Phiên active</span><span>Trạng thái</span><span>Thao tác</span></div>
    {visibleClients.map((client) => {
      ...
      return <article key={client.id} className={styles.applicationRow}>
        ...
        {client.href ? <Link href={client.href} className={styles.manageButton}>Vào quản trị →</Link> : <span className={styles.connectionState} data-state="pending"><i />Chưa nối production</span>}
      </article>;
    })}
  ...
  const workTable = <div className={styles.workTable}>
    ...
    {workItems.map((item) => <article key={item.id} className={styles.workRow}>
      ...
      {item.client.href ? <Link href={item.client.href} className={styles.rowAction}>Xử lý →</Link> : <span className={styles.connectionState} data-state="pending">Chờ backend</span>}
    </article>)}
  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.main}>
      <header className={styles.topbar}>
        <label className={styles.searchBox}><span>⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Tìm theo ứng dụng, thiết bị, nghiệp vụ…" /></label>
        <select className={styles.filterSelect} value={appFilter} onChange={(event) => setAppFilter(event.target.value)}><option value="all">Tất cả ứng dụng</option>{clients.map((client) => <option key={client.id} value={client.id}>{client.shortName}</option>)}</select>
      ...
      <div className={styles.pageBody}>
        <header className={styles.pageHeader}><div><span>{title.eyebrow}</span><h1>{title.title}</h1><p>{title.description}</p></div><button className={styles.syncButton} onClick={() => void initialize()} disabled={busy}>{busy ? "Đang đồng bộ…" : "Đồng bộ"}</button></header>
        ...
        {view === "overview" ? <>
          <section className={styles.metricGrid}>
            <button onClick={() => switchView("applications")} data-tone="teal"><i>◇</i><div><span>Tổng ứng dụng</span><strong>{clients.length}</strong><small>Client cấp 1 trong topology</small></div><b>→</b></button>
            <button onClick={() => switchView("client-devices")} data-tone="amber"><i>▯</i><div><span>Thiết bị mới chờ duyệt</span><strong>{pendingClientDevices}</strong><small>Dữ liệu client đang trả về</small></div><b>→</b></button>
            ...
            <button onClick={() => switchView("inbox")} data-tone="gold"><i>▤</i><div><span>Việc cần xử lý</span><strong>{workItems.length}</strong><small>QT + client, không tạo số giả</small></div><b>→</b></button>
          </section>
          <section className={styles.dashboardGrid}>
            ...
            <div className={styles.panel}><SectionHeader title="Cảnh báo nhanh" meta={`${connectedClients}/${clients.length} client kết nối`} action={<button onClick={() => switchView("alerts")}>Xem tất cả →</button>} /><div className={styles.alertTiles}>
              ...
              <button onClick={() => switchView("devices")} data-tone="gold"><span>♙</span><div><small>Thiết bị QT chờ duyệt</small><strong>{centralCounts.pending}</strong><em>Chỉ registry Trung tâm</em></div></button>
        ...
        {view === "client-devices" ? <section className={styles.panel}><SectionHeader title="Thiết bị mới theo ứng dụng" meta="Registry vẫn thuộc client" /><div className={styles.applicationTable}>
          <div className={styles.applicationHead}><span>Ứng dụng</span><span>Nhóm</span><span>Thiết bị chờ</span><span>Phiên active</span><span>Trạng thái</span><span>Thao tác</span></div>
          {visibleClients.map((client) => { const row = clientRows.find((item) => item.client.id === client.id)!; return <article key={client.id} className={styles.applicationRow}><div className={styles.appCell}><b>{client.icon}</b><div><strong>{client.shortName}</strong><small>{client.boundary}</small></div></div><span>{client.group}</span><strong data-count={(row.pending ?? 0) > 0 ? "attention" : "normal"}>{row.pending ?? "—"}</strong><strong>{row.active ?? "—"}</strong><StatusDot state={row.state}/>{client.href ? <Link href={client.href} className={styles.manageButton}>Mở quản trị →</Link> : <span className={styles.connectionState} data-state="pending">Chưa có adapter</span>}</article>; })}
        </div></section> : null}
        ...
        {view === "settings" ? <section className={styles.settingsGrid}>
          ...
          <div className={`${styles.panel} ${styles.boundaryPanel}`}><SectionHeader title="Ranh giới nghiệp vụ" /><div className={styles.boundaryCards}><article data-client="health"><strong>Sức khỏe Y tế</strong><p>Chỉ quản trị Health: thiết bị SK, policy, session, kiểm duyệt nội dung y tế và audit ứng dụng. Không dùng registry Bơi ếch hay Hòa nhập Nga.</p></article><article data-client="ru"><strong>Hòa nhập Nga</strong><p>Là client độc lập: thiết bị HN, OCR thuốc, đối chiếu quy định và audit Nga. Production cũ chưa có adapter nên Trung tâm không dựng nút thao tác giả.</p></article></div></div>
        </section> : null}
      </div>
    </section>
  </main>;
