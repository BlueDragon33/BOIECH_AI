...
export default function BaumanAdminClient({ user }: { user: { displayName: string; email: string } }) {
  ...
  async function load() {
    ...
    try {
      ...
      if (nextStatus.readiness?.deviceRegistry === "available" && nextStatus.readiness?.deviceGateway === "available") {
  ...
  return <>
    <section className={styles.summaryGrid}>
      <article className={styles.summaryCard}><span>Thiết bị Bauman</span><strong>{deviceReviewReady ? devices.length : "—"}</strong><small>{deviceReviewReady ? "Registry thuộc Bauman" : "Registry chưa nối"}</small></article>
    ...
    {tab === "devices" ? <section className={styles.panel}>
      ...
      {!deviceReviewReady ? <div className={styles.error}><strong>Không dựng nút Duyệt/Loại bỏ giả.</strong><br/>Chỉ khi Bauman Control Service công bố `deviceRegistry` + `deviceGateway` ở trạng thái `available`, danh sách và thao tác thật mới được mở.</div> : null}
      {deviceReviewReady ? <div className={styles.versionList}>{devices.map((device) => <article className={styles.versionRow} key={device.deviceId}>
        ...
        <div className={styles.versionMeta}><strong>{device.displayName || device.label || device.deviceCode}</strong><small>{device.deviceCode}{device.platform || device.browser ? ` · ${[device.platform, device.browser].filter(Boolean).join(" · ")}` : ""}{device.lastSeenAt ? ` · tín hiệu ${new Date(device.lastSeenAt).toLocaleString("vi-VN")}` : ""}</small></div>
        <div className={styles.actionBar}>{device.status === "pending" ? <><button className={styles.primaryButton} disabled={deviceBusy === device.deviceId || access.role !== "owner"} onClick={() => void manageDevice(device, "approve")}>{deviceBusy === device.deviceId ? "Đang xử lý…" : "Duyệt"}</button><button className={styles.dangerButton} disabled={deviceBusy === device.deviceId || access.role !== "owner"} onClick={() => void manageDevice(device, "reject")}>Loại bỏ</button></> : <span>{device.status === "approved" ? (device.active ? "Online · đã duyệt" : "Đã duyệt") : "Đã chặn"}</span>}</div>
