...
export default function HealthControlCenterV2({ user }: { user: { displayName: string; email: string } }) {
  ...
  return <main className={styles.shell}><div className={styles.frame}>
    ...
    <section className={styles.summary}>
      <article><span>Dịch vụ</span><strong className={controlStatus?.service === "paused" ? styles.paused : styles.online}>{controlStatus?.service === "paused" ? "Tạm dừng" : "Online"}</strong><small>Contract v{controlStatus?.contractVersion ?? "—"}</small></article>
    ...
    {tab === "overview" ? <section className={styles.panel}>
      <div className={styles.panelHead}><div><span className={styles.eyebrow}>Kết nối hai hệ thống</span><h2>Trạng thái Control Plane</h2><p>Đây là lớp kết nối, không phải dashboard sức khỏe của trẻ.</p></div><div className={styles.actor}><strong>{user.displayName}</strong><small>{roleLabels[access.role]} · {access.deviceCode}</small></div></div>
