...
function ageLabel(birthDate: string, now = new Date()) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(birthDate)) return { text: "Chưa có ngày sinh", inScope: null as boolean | null };
  const birth = new Date(`${birthDate}T12:00:00`);
  if (!Number.isFinite(birth.getTime()) || birth > now) return { text: "Ngày sinh chưa hợp lệ", inScope: null as boolean | null };
  let months = (now.getFullYear() - birth.getFullYear()) * 12 + now.getMonth() - birth.getMonth();
  if (now.getDate() < birth.getDate()) months -= 1;
  months = Math.max(0, months);
  const years = Math.floor(months / 12);
  const rest = months % 12;
  return { text: `${years} tuổi${rest ? ` ${rest} tháng` : ""}`, inScope: years === 9 || years === 10 };
}
...
export default function HealthFramework({ initialCourse, device }: { initialCourse: unknown; device: HealthDeviceAccess }) {
  ...
  const profileAge = useMemo(() => ageLabel(state.profile.birthDate), [state.profile.birthDate]);
  ...
  function downloadBackup() {
    ...
    anchor.href = url;
    anchor.download = `suc-khoe-y-te-backup-${today}.json`;
    document.body.appendChild(anchor);
    anchor.click();
    anchor.remove();
    URL.revokeObjectURL(url);
    setBackupNotice("Đã tạo bản sao dữ liệu cục bộ. Hãy lưu tệp ở nơi an toàn.");
  }
  ...
  async function importBackup(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setState(imported);
      setDayKey(today);
      setBackupNotice("Đã khôi phục bản sao vào trình duyệt này.");
    } catch (error) {
  ...
  return <main className="hf-shell" data-content-ready={contentReady ? "true" : "false"}>
    <div className="hf-layout">
      ...
      <section className="hf-content">
        <div className="hf-topbar"><div><span className="hf-kicker">Bộ khung vận hành · local-first</span><h1>Sức khỏe Y tế 9–10 tuổi</h1></div><div className="hf-top-status"><span className="hf-dot" />{contentReady ? `${device.deviceCode} · đã cấp truy cập` : "Đang chờ nội dung"}</div></div>
        ...
        {active === "today" ? <section className="hf-section">
          <SectionHeader title={dayKey === today ? "Hôm nay" : "Ngày đã chọn"} description="Checklist, tiến độ và nhắc việc được gom vào một màn hình. Có thể lùi về ngày trước để xem hoặc bổ sung nhật ký." aside={hydrated ? formatDate(dayKey) : "Đang đọc dữ liệu…"} />
        ...
        {active === "growth" ? <section className="hf-section">
          <SectionHeader title="Tăng trưởng" description="Lưu từng lần đo chiều cao và cân nặng. BMI hiển thị như giá trị tính toán; chưa tự xếp loại vì đánh giá trẻ cần xét tuổi và giới tính." aside={profileAge.text} />
        ...
        {active === "nutrition" ? <section className="hf-section">
          <SectionHeader title="Dinh dưỡng" description="Checklist nhóm thực phẩm, nước và nhật ký bữa ăn theo ngày. Không dùng calorie tracker hoặc mục tiêu giảm cân người lớn." aside={formatDate(dayKey)} />
          <div className="hf-work-grid">
        ...
        {active === "activity" ? <section className="hf-section">
          <SectionHeader title="Vận động" description="Ghi loại hoạt động và số phút trong ngày để theo dõi thói quen theo thời gian." aside={formatDate(dayKey)} />
        ...
        {active === "care" ? <section className="hf-section">
          <SectionHeader title="Chăm sóc" description="Giấc ngủ, răng miệng, mắt & học tập và vệ sinh cá nhân được gom trong một khu vực." aside={formatDate(dayKey)} />
        ...
        {active === "journal" ? <section className="hf-section">
          <SectionHeader title="Nhật ký" description="Ghi cảm nhận và triệu chứng để theo dõi diễn biến. Đây không phải công cụ tự chẩn đoán hoặc tự kê đơn." aside={formatDate(dayKey)} />
          <section className="hf-panel"><div className="hf-panel-head"><div><span className="hf-kicker">Cảm nhận</span><h3>Em cảm thấy</h3></div></div><div className="hf-choice-row">{([['good','Khỏe'],['normal','Bình thường'],['unwell','Không khỏe']] as const).map(([value, label]) => <button type="button" key={value} className={day.feeling === value ? "hf-choice is-active" : "hf-choice"} onClick={() => updateDay((current) => ({ ...current, feeling: value }))}>{label}</button>)}</div></section>
        ...
        {active === "profile" ? <section className="hf-section">
          ...
          <div className="hf-work-grid">
            <section className="hf-panel"><div className="hf-panel-head"><div><span className="hf-kicker">Hồ sơ trẻ</span><h3>Thông tin cơ bản</h3></div><span className={profileAge.inScope === false ? "hf-scope-badge is-warning" : "hf-scope-badge"}>{profileAge.text}</span></div><div className="hf-form-grid"><label>Tên / tên gọi<input value={state.profile.name} maxLength={80} onChange={(event) => setState((current) => ({ ...current, profile: { ...current.profile, name: event.target.value } }))} /></label><label>Ngày sinh<input type="date" max={today} value={state.profile.birthDate} onChange={(event) => setState((current) => ({ ...current, profile: { ...current.profile, birthDate: event.target.value } }))} /></label><label>Giới tính<select value={state.profile.sex} onChange={(event) => setState((current) => ({ ...current, profile: { ...current.profile, sex: event.target.value as "male" | "female" | "" } }))}><option value="">Chưa chọn</option><option value="male">Nam</option><option value="female">Nữ</option></select></label></div>{profileAge.inScope === false ? <div className="hf-age-warning">Hồ sơ hiện nằm ngoài trọng tâm 9–10 tuổi. Ứng dụng vẫn cho ghi dữ liệu, nhưng nội dung khuyến nghị chuyên sâu sau này sẽ bám đúng nhóm tuổi đã cấu hình.</div> : null}<label className="hf-textarea-label">Ghi chú cần nhớ<textarea value={state.profile.note} maxLength={800} onChange={(event) => setState((current) => ({ ...current, profile: { ...current.profile, note: event.target.value } }))} /></label><div className="hf-local-badge">Chỉ lưu trong trình duyệt hiện tại · không gửi hồ sơ này sang Site Quản trị.</div></section>
